...
class PredictiveSearchComponent extends Component {
  ...
  connectedCallback() {
    ...
    this.addEventListener('click', this.#handleKeywordClick, { signal });
  ...
  /**
   * Handles keyword button clicks to populate search and trigger results
   * @param {MouseEvent} event - The mouse event
   */
  #handleKeywordClick = (event) => {
    const keywordButton = /** @type {HTMLElement} */ (event.target).closest('.predictive-search-keyword');
    if (!keywordButton || !(keywordButton instanceof HTMLElement)) return;

    const keyword = keywordButton.dataset.keyword;
    if (!keyword || !this.refs.searchInput) return;
    ...
    this.refs.searchInput.value = searchTerm;
